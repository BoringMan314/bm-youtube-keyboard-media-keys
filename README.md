...
請在 [Chrome Web Store](https://chromewebstore.google.com/) 搜尋 **「[[B.M] YouTube 支援鍵盤媒體鍵](https://chromewebstore.google.com/detail/bm-youtube-%E6%94%AF%E6%8F%B4%E9%8D%B5%E7%9B%A4%E5%AA%92%E9%AB%94%E9%8D%B5/omadcnhemhgioppikilldhakofffpejm?hl=zh-TW)」**，或直接點選連結前往商店頁面安裝。

### 從原始碼載入（開發人員模式）

1. 點選本頁綠色 **Code** → **Download ZIP** 解壓，或使用 Git 複製：`git clone https://github.com/BoringMan314/bm-youtube-keyboard-media-keys.git`。
...
本擴充功能**不蒐集、不上傳**任何個人資料或瀏覽記錄；未使用任何分析工具或遠端程式碼。僅於本機 `storage` 儲存開關狀態。
...
歡迎透過 [GitHub Issues](https://github.com/BoringMan314/bm-youtube-keyboard-media-keys/issues) 回報錯誤或提出改善建議（回報時請提供瀏覽器版本、介面語言與重現步驟）。
