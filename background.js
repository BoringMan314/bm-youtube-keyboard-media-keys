				...
				func: () => {
					...
					if (path.startsWith('/shorts/')) {
						...
						if (!v) {
							...
							const inner =
								flexy?.querySelector('#movie_player video') ??
								flexy?.querySelector('ytd-player video');
							v = inner ?? null;
						}
						if (!v) {
							const ytdPlayer =
		...
		await chrome.scripting.executeScript({
			...
			func: (next) => {
				...
				function closeShortsEngagementIfOpen() {
					...
					const visBtn =
						shorts.querySelector(
							'ytd-engagement-panel-title-header-renderer #visibility-button button'
						) ||
						shorts.querySelector(
							'ytd-engagement-panel-title-header-renderer tp-yt-paper-icon-button#button'
						) ||
